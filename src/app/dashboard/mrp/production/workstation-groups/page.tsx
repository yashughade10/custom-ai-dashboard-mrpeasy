...
import WorkstationGroupsTable from "@/components/production/WorkstationGroupsTable";
import WorkstationGroupForm from "@/components/production/WorkstationGroupForm";
...
function WorkstationGroupsPage() {
          ...
          <>
            <div className="px-4 py-3 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3 bg-white">
              <div className="flex items-center gap-3">
                <h1 className="text-xl font-bold text-gray-900">Workstation groups</h1>
                <Button 
                  size="sm" 
            ...
            <div className="px-4 py-3 flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 overflow-x-auto min-h-0 border border-gray-200 rounded-sm">
                <WorkstationGroupsTable onEdit={(id) => setEditingId(id)} />
              </div>
            </div>
          </>
        )}
...
export default function WorkstationGroupsPageGuarded() {
  return (
    <RouteGuard module="production">
      <React.Suspense fallback={<div className="p-8 text-center">Loading...</div>}>
        <WorkstationGroupsPage />
      </React.Suspense>
    </RouteGuard>
  );
}
