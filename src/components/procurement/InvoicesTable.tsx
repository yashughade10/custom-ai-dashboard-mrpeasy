...
import { CalendarDays, Plus, Settings2, Flag, Edit2, ChevronDown, Loader2, Save, X, BarChart2, Download } from "lucide-react";
...
const ALL_COLUMNS = [
  ...
  { id: 'purchase_order_col', label: 'Purchase order' },
...
export default function InvoicesTable() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mrp_invoice_saved_searches');
    if (saved) {
      try {
  ...
  const saveCurrentSearch = () => {
    ...
    localStorage.setItem('mrp_invoice_saved_searches', JSON.stringify(newSearches));
    setNewSearchName("");
  ...
  return (
    <div className="w-full bg-white text-[12px] text-gray-800 flex flex-col flex-1 min-h-0 relative print:p-0 print:m-0 print:block">
      ...
      {/* Print Header */}
      <div className="flex justify-between items-center px-4 py-2 bg-[#f9fafb] border-b border-gray-200">
        <div className="flex items-center gap-4">
          <h1 className="text-[22px] font-semibold text-[#1e293b] m-0 p-0">Incoming invoices</h1>
        </div>
        
        <div className="print:hidden flex items-center gap-2">
          <button onClick={handleDownloadPDF} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#e4e9f0] hover:bg-[#d1d8e4] text-gray-700 rounded transition-colors border-none cursor-pointer font-medium text-[12px]">
            <Download className="w-3.5 h-3.5" />
            PDF
          </button>
          <button onClick={handleDownloadCSV} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#e4e9f0] hover:bg-[#d1d8e4] text-gray-700 rounded transition-colors border-none cursor-pointer font-medium text-[12px]">
            <Download className="w-3.5 h-3.5" />
            CSV
          </button>
        </div>
        ...
        <table className="w-full text-left table-auto print:text-[10px]">
          <thead className="bg-[#f0f4f8] text-gray-700 font-semibold border-b border-gray-300">
            ...
            <tr>
              ...
              {ALL_COLUMNS.map(col => {
                ...
                if (col.id === 'po_number') {
                  return (
                    <th key={col.id} className={`font-medium p-2 leading-tight flex items-center gap-1 border-r border-gray-300 border-b border-gray-300 whitespace-nowrap${extraClass}`}>
                ...
                return (
                  <th key={col.id} className={`font-medium p-2 leading-tight border-r border-gray-300 border-b border-gray-300 whitespace-nowrap${extraClass}`}>
                    {col.label}
                  </th>
                );
              })}
              <th className="print:hidden font-medium p-2 w-16 text-center border-b border-gray-300 relative">
                <div 
            ...
            <tr className="print:hidden bg-white border-b border-gray-300">
              ...
              {ALL_COLUMNS.map(col => {
                ...
                if (['total', 'tax', 'total_including_tax', 'paid', 'unpaid', 'total_quantity', 'total_in_currency', 'tax_in_currency', 'total_including_tax_in_currency', 'paid_in_currency', 'unpaid_in_currency', 'delay'].includes(col.id)) {
                  return (
                    <th key={col.id} className="p-1.5 align-top border-r border-gray-300 font-normal space-y-1 w-24">
                      <input name={`min_${col.id}`} value={(filters as any)[`min_${col.id}`]} onChange={handleNumericChange} onBlur={handleBlur} onKeyDown={handleKeyDown} type="text" placeholder="min" className="w-full h-7 bg-[#F3F4F6] border-none rounded-sm px-2 focus:ring-1 focus:ring-blue-500 outline-none text-[11px] placeholder-gray-400" />
                      <input name={`max_${col.id}`} value={(filters as any)[`max_${col.id}`]} onChange={handleNumericChange} onBlur={handleBlur} onKeyDown={handleKeyDown} type="text" placeholder="max" className="w-full h-7 bg-[#F3F4F6] border-none rounded-sm px-2 focus:ring-1 focus:ring-blue-500 outline-none text-[11px] placeholder-gray-400" />
                    </th>
                  );
                }

                if (['created_date', 'expected_date', 'arrival_date', 'order_date', 'invoice_date', 'due_date', 'shipped_on'].includes(col.id)) {
                  const prefix = col.id.replace('_date', '').replace('shipped_on', 'shipped_on'); // simplistic
                  const minKey = `min_${prefix === 'created' ? 'created' : prefix === 'expected' ? 'expected' : prefix === 'arrival' ? 'arrival_date' : prefix === 'order' ? 'order_date' : prefix === 'invoice' ? 'invoice_date' : prefix === 'due' ? 'due_date' : 'shipped_on'}`;
                  const maxKey = `max_${prefix === 'created' ? 'created' : prefix === 'expected' ? 'expected' : prefix === 'arrival' ? 'arrival_date' : prefix === 'order' ? 'order_date' : prefix === 'invoice' ? 'invoice_date' : prefix === 'due' ? 'due_date' : 'shipped_on'}`;
                  return (
                    <th key={col.id} className="p-1.5 align-top border-r border-gray-300 font-normal space-y-1 w-[130px]">
                      <input name={minKey} value={(filters as any)[minKey]} onChange={handleChange} onKeyDown={handleKeyDown} type="date" className="w-full h-7 bg-[#F3F4F6] border-none rounded-sm px-2 focus:ring-1 focus:ring-blue-500 outline-none text-[11px] text-gray-500" />
                      <input name={maxKey} value={(filters as any)[maxKey]} onChange={handleChange} onKeyDown={handleKeyDown} type="date" className="w-full h-7 bg-[#F3F4F6] border-none rounded-sm px-2 focus:ring-1 focus:ring-blue-500 outline-none text-[11px] text-gray-500" />
                    </th>
                  );
                }

                if (['status', 'currency', 'invoice_status', 'payment_status', 'created_by', 'po_emailed', 'attention', 'account'].includes(col.id)) {
                  let options: React.ReactNode = <option value=""></option>;
                  if (col.id === 'status') {
                    options = (
                      <>
                        <option value=""></option>
                  ...
                  } else if (col.id === 'po_emailed' || col.id === 'account') {
                    options = (
                      <>
                        <option value=""></option>
                  ...
                  return (
                    <th key={col.id} className="p-1.5 align-top border-r border-gray-300 font-normal w-28">
                      <div className="relative mt-2">
                        <select 
                          ...
                          onChange={(e) => {
                            ...
                            setFilters(prev => ({ ...prev, [col.id]: val }));
                            setActiveFilters(prev => ({ ...prev, [col.id]: val }));
                            setLimit(50);
                          }} 
                ...
                // Default text input
                // Mapped id exception (e.g. purchase_order_col filters by po_number technically? Wait, we don't have separate filter for this, just use col.id)
                return (
              ...
              <th className="p-1.5 align-top bg-[#f9fafb]">
                <div className="flex items-center justify-end gap-1 pr-1 pt-1">
                  <button onClick={handleSearch} className="px-2 py-1.5 bg-[#f0f4fc] text-[#1e5aa0] hover:bg-[#e4ebf7] rounded font-medium text-[11px] transition-colors cursor-pointer border-none">Search</button>
                  <button onClick={handleClear} className="text-[#1e5aa0] hover:underline text-[11px] font-medium bg-transparent border-none cursor-pointer px-1">Clear</button>
                </div>
              </th>
            </tr>
            {/* Totals Row */}
          ...
          <tbody>
            ...
            {isLoading && limit === 50 && orders.length === 0 ? (
              <tr>
                <td colSpan={Object.values(visibleCols).filter(Boolean).length + 2} className="text-center py-12 text-gray-500">Loading invoices...</td>
              </tr>
            ) : orders.length === 0 ? (
              <tr>
                <td colSpan={Object.values(visibleCols).filter(Boolean).length + 2} className="px-4 py-8 text-center text-gray-500">No data available</td>
              </tr>
            ) : orders.map((order: any, i: number) => (
              <tr key={order.id || i} onClick={() => router.push(`/dashboard/mrp/procurement/${order.po_number}`)} className="hover:bg-gray-50 border-b border-gray-200 cursor-pointer text-gray-600">
                <td className="p-2 text-center text-gray-400 border-r border-gray-200">{i + 1}</td>
                {ALL_COLUMNS.map(col => {
                  if (!visibleCols[col.id]) return null;

                  let content: React.ReactNode = order[col.id];

                  if (col.id === 'created_date' || col.id === 'invoice_date' || col.id === 'due_date' || col.id === 'expected_date' || col.id === 'arrival_date' || col.id === 'order_date' || col.id === 'shipped_on') {
                    content = formatShortDate(order[col.id]);
                  ...
                  } else if (col.id === 'purchase_order_col') {
                    content = order.po_number;
                  } else if (col.id === 'total' || col.id === 'tax' || col.id === 'total_including_tax' || col.id === 'paid' || col.id === 'unpaid' || col.id === 'total_in_currency' || col.id === 'tax_in_currency' || col.id === 'total_including_tax_in_currency' || col.id === 'paid_in_currency' || col.id === 'unpaid_in_currency') {
                    const curr = order.currency || '$';
                    return <td key={col.id} className="p-2 border-r border-gray-200 whitespace-nowrap text-right font-medium">{curr} {formatCurrency(order[col.id] || 0)}</td>;
                  } else if (col.id === 'tax_rate') {
                    content = order.tax_rate != null ? `${order.tax_rate}%` : '10%';
                  } else if (col.id === 'xero') {
                    content = order.xero ? 'Yes' : 'No';
                  } else if (col.id === 'currency') {
                    return <td key={col.id} className="p-2 border-r border-gray-200 whitespace-nowrap text-center">{order.currency}</td>;
                  } else if (col.id === 'invoice_status' || col.id === 'payment_status') {
                    content = getStatusBadge(order[col.id]);
                  } else if (col.id === 'delay') {
                    content = order.delay != null && order.delay !== '' ? `${order.delay} d` : '';
                  }

                  return <td key={col.id} className="p-2 border-r border-gray-200 whitespace-nowrap">{content}</td>;
                })}
